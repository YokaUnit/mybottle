        ...
        {(tab === "all" || tab === "news") &&
          (initialNews.length === 0 ? (
            <article className="mb-surface p-5">
              <p className="text-sm font-medium text-[var(--mb-forest-light)]">お知らせはまだありません。</p>
            </article>
          ) : (
            initialNews.map((item) => (
              <article key={item.id} className="mb-surface p-5">
                {item.badgeLabel ? (
                  <p className="text-xs font-semibold text-[var(--mb-accent-dark)]">{item.badgeLabel}</p>
                ) : null}
                <p className="mt-2 font-semibold tracking-[-0.02em] text-[var(--mb-ink)]">{item.title}</p>
                <p className="mt-1.5 text-sm font-medium leading-relaxed text-[var(--mb-forest-light)]">{item.body}</p>
              </article>
            ))
          ))}
      </div>
